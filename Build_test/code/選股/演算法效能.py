...
for propro in range(6,24):
    ...
    for i in range(df_reward_std.shape[0]):
        ...
        mazda.the_sharp = float(df_reward_std['3'][i]) / float(df_reward_std['4'][i])
    ...
    total_risk.append(sql_min_risk)
    total_reward.append(sql_max_reward)
    total_ratio.append(sql_final_w)
    total_name.append(sql_final_name)
    total_expect.append(sql_expect_reward)


total = pd.DataFrame(total_ratio,columns=['ratio'])
...
total = pd.concat([total, pd.DataFrame(total_expect,columns=['expect'])],axis=1)
# print(total['expect_r'])
# print(exp_rw[-1])
str_data = 'C:/Users/User/Downloads/資產配置/演算法效能_etf_20_5_compare.csv'
