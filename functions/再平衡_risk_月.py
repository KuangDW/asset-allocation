...
cursor = db.cursor()
sql = "select * from `user_data` where (name = '"+ username +"' and id = '" + password +"')"
cursor.execute(sql)
...
while((final_date)<todayyy):
    ...
    new_asset = new_asset.replace('[','')
    new_asset = new_asset.replace(']','')    
    sql = "UPDATE user_data SET last_money = '"+ str(new_asset) +"' WHERE name = '" +username+"'; "
    # print(sql)
    cursor.execute(sql)
    result_select = cursor.fetchall()
    db.commit()
    
    sql = "UPDATE user_data SET last_time = '"+ str(final_date) +"' WHERE name = '" +username+"'; "
    # print(sql)
    cursor.execute(sql)
    result_select = cursor.fetchall()
    db.commit()
    ...
    sell_buy = sell_buy.replace('[','')
    sell_buy = sell_buy.replace(']','') 
    sql = "UPDATE user_data SET sell_buy = '"+ str(sell_buy) +"' WHERE name = '" +username+"'; "
    # print(sql)
    cursor.execute(sql)
    result_select = cursor.fetchall()
    db.commit()
    ...
    after_adj_ratio = after_adj_ratio.replace('[','')
    after_adj_ratio = after_adj_ratio.replace(']','') 
    sql = "UPDATE user_data SET last_ratio = '"+ str(after_adj_ratio) +"' WHERE name = '" +username+"'; "
    # print(sql)
    cursor.execute(sql)
    result_select = cursor.fetchall()
    db.commit()
